...
- Rebuild/restart app to apply updates (`npm run dev` auto-reloads in local)

Argo deployment structure:

- Helm app chart: `.deployment/home/`
- Argo Events (GitHub webhook): `.deployment/events/`
- Argo Workflows templates/RBAC: `.deployment/workflows/`

Before enabling webhooks/workflows, update placeholders:

- `.deployment/events/values.yaml` -> `webhook`
- `.deployment/events/k3s-master-values.yaml` -> `webhook`
